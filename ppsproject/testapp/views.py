...
from .models import  Subject,Register,Quiz,AdminRegister,ApplicationFormClass,ExamCode,SubjectCode,Score
...
def login1(request):
    if request.method=='POST':
        ...
        try:
            ...
            return  render(request,'testapp/login.html',{"msg":"username and password does not match"})
...
def onlineofflinecount(request):
    on  = ApplicationFormClass.objects.filter(status='online').count()
...
def gettest(request):
    ...
    return  render(request,'testapp/a.html',{'questions':s,'query':query,'a':a})
